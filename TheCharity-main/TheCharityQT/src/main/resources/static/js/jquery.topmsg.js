(function(){
	...
	jQuery.topmsg=function(msg,sleep,param,animate){
		...
		if(isNaN(sleep)){
                ...
                if(param.icon_color==undefined){
                	...
                	if(window.localStorage){
                		icbgcc=localStorage.getItem('topmsg_icbgcc');
                	}
                	if(icbgcc=='rgb(255, 255, 255)'){
                		icbgcc='#333';
                	}
                    param.icon_color=(icbgcc?icbgcc:'#eb586f');
                }
		...
		' 	<div id="sl_topmsg_body" class="flash-base my-not-act"><div class="js_closethistipbtn" style=""> × </div>'+load_icon+'<icon style="float:left;font-size:28px;color:'+param.icon_color+'" class="iconfont iconclass '+param.icon+' "></icon><div style="float:left;padding:5px;width:100%;"  class="html_box"></div><div style="clear:both;"></div></div>'+
		'</div></div>';
			var d=$('.sl_topmsg');
			if(d.length==0){
				d=$(tpl);
